...
class Scheduler:
    ...
    def schedule(
        self, func: Optional[Callable] = None, *, interval: Union[timedelta, float]
    ) -> Callable:
        """
        ...
        self._tasks.append(_Task(func, interval))
    ...
    def run_loop(
        ...
        while not stop_event.is_set() and not monotonic() - start_time > return_after:
            ...
            next_call_time_list = [t.next_call() for t in self._tasks]
            if len(next_call_time_list):
                next_call_time = min([t.next_call() for t in self._tasks]) - monotonic()
            else:
